...
parser.add_argument('--num_chain_per_target', type=int, default=2)
...
NUM_CHAIN_PER_TARGET = args.num_chain_per_target
NUM_CHAIN = NUM_TARGET * NUM_CHAIN_PER_TARGET
...
if len(batch)<NUM_TARGET:
    batch = batch.repeat((NUM_TARGET//len(batch)+1, 1, 1, 1))
batch = batch[:NUM_TARGET]

# Targets
rphi_true = sample_prior(NUM_TARGET).to(device)
...
sigmas = torch.rand(NUM_TARGET, 1).to(device) * (NOISE_LEVEL_MAX - NOISE_LEVEL_MIN) + NOISE_LEVEL_MIN
...
# Split the different batchs then repeat for each chain per target

noisy_batch = torch.split(noisy_batch, 1, dim=0)
noisy_batch = [batch.repeat((NUM_CHAIN_PER_TARGET, 1, 1, 1)) for batch in noisy_batch]
noisy_batch = torch.cat(noisy_batch, dim=0)

mean_batch = torch.split(mean_batch, 1, dim=0)
mean_batch = [batch.repeat((NUM_CHAIN_PER_TARGET, 1, 1, 1)) for batch in mean_batch]
mean_batch = torch.cat(mean_batch, dim=0)

noise_batch = torch.split(noise_batch, 1, dim=0)
noise_batch = [batch.repeat((NUM_CHAIN_PER_TARGET, 1, 1, 1)) for batch in noise_batch]
noise_batch = torch.cat(noise_batch, dim=0)

rphi_true = torch.split(rphi_true, 1, dim=0)
rphi_true = [batch.repeat((NUM_CHAIN_PER_TARGET, 1)) for batch in rphi_true]
rphi_true = torch.cat(rphi_true, dim=0)

phi_true = torch.split(phi_true, 1, dim=0)
phi_true = [batch.repeat((NUM_CHAIN_PER_TARGET, 1)) for batch in phi_true]
phi_true = torch.cat(phi_true, dim=0)

ps_true = torch.split(ps_true, 1, dim=0)
ps_true = [batch.repeat((NUM_CHAIN_PER_TARGET, 1, 1, 1)) for batch in ps_true]
ps_true = torch.cat(ps_true, dim=0)

sq_ps_true = torch.split(sq_ps_true, 1, dim=0)
sq_ps_true = [batch.repeat((NUM_CHAIN_PER_TARGET, 1, 1, 1)) for batch in sq_ps_true]
sq_ps_true = torch.cat(sq_ps_true, dim=0)

sigmas = torch.split(sigmas, 1, dim=0)
sigmas = [batch.repeat((NUM_CHAIN_PER_TARGET, 1)) for batch in sigmas]
sigmas = torch.cat(sigmas, dim=0)

timesteps = torch.split(timesteps, 1, dim=0)
timesteps = [batch.repeat((NUM_CHAIN_PER_TARGET, 1)) for batch in timesteps]
timesteps = torch.cat(timesteps, dim=0)

## Rescaling the noisy batch
